function loadUser() {
		...
		var user = $.trim($('input[name=fullname]').val());
        if($.trim(user).indexOf(' ') +1){
            user = $.trim(user).substring(0,$.trim(user).indexOf(' '));
        }
		$.ajax({
			url: "http://10.93.105.49/soap_services/soap.php",
			type: 'POST',
			data: {"lastName": user},
			dataType: 'jsonp',
			success: function(data) {
                ...
                $('.apply').click(function () {
                    ...
                    if (person) {
                        ...
                        $('form input[type="text"],form input[type="password"]').css({border: "solid 1px #ccc"});
...
$(document).ready(function(){
    $('.passForActiveDirectory').keyup(function(e){
        if(e.keyCode == 13){
    ...
    $('#fullname').keyup(function(e){
        ...
        $('.active-directory').click(function(){
            ...
            $.ajax({
                type: "POST",
                url: "http://10.93.105.49/soap_services/ldap.php",
