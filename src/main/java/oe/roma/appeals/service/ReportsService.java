...
public class ReportsService {
    ...
    public List<Appeal> getListOnControlD4(int month, int year){
        ...
        String sql = mainSql+" WHERE /*YEAR(r.deadline_term)="+year+" AND*/ r.in_date<=LAST_DAY("+date+")  AND (r.out_date IS NULL OR r.out_date>LAST_DAY("+date+"))";//AND MONTH(r.in_date)<="+month+"
        return jdbcTemplate.query(sql, appealsService.mapper);
    }
    public List<Appeal> getListOffControlD3(int month, int year){

        String sql = mainSql+" where month(r.out_date)="+month+" and year(r.out_date)="+year+" and (r.out_date is not null or r.out_number is not null or r.text_answer is not null) ";
        return jdbcTemplate.query(sql, appealsService.mapper);
    }
    public List<Appeal> getListExpiredD5(int month, int year){
        //String date = "'"+year + "-" +month + "-01'";
        String sql = mainSql+" where YEAR(r.deadline_term)="+year+" and MONTH(r.deadline_term)="+month+" and (r.out_date>r.deadline_term or r.out_date is null) ";
        return jdbcTemplate.query(sql, appealsService.mapper);
    }
    ...
    public Map getListTemp(int month, int year){
        ...
        String sql ="" +
                ...
                "(SELECT COUNT(*) FROM registry r WHERE /*YEAR(r.deadline_term)="+year+" AND */ r.in_date<=LAST_DAY("+date+")  AND (r.out_date IS NULL OR r.out_date>LAST_DAY("+date+"))) AS COUNT";
