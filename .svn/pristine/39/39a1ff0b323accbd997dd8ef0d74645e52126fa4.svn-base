...
public class AppealsService {
    ...
    public List<TypeAppeal> getTypeAppeals() {
        ...
        String sql = "SELECT id, name, term_to_answer, reminder_term FROM type_appeals";

        return jdbcTemplate.query(sql, ta_mapper);
    }

    public TypeAppeal getTypeAppealById(Integer id) {

        String sql = "SELECT id, name, term_to_answer, reminder_term FROM type_appeals where id=?";

        TypeAppeal typeAppeal = (TypeAppeal) jdbcTemplate.queryForObject(
                sql, new Object[]{id}, ta_mapper);
        return typeAppeal;
    }
    ...
    public void editTypeAppeal(TypeAppeal typeAppeal) {
        String sql = "update type_appeals set "
                + "name = ?, "
                + "term_to_answer = ?, "
                + "reminder_term = ? "
                + "where id = ?";
    ...
    public List<Signature> getSignatures() {
        ...
        return jdbcTemplate.query(sql, signaturemapper);
    }
    ...
    public Signature getSignatureById(Integer id) {
        ...
        String sql = "SELECT id, signature FROM signatures where id=?";

        Signature signature = (Signature) jdbcTemplate.queryForObject(
                sql, new Object[]{id}, signaturemapper);
        return signature;
    }
    ...
    public void add(Appeal appeal) {
        ...
        jdbcTemplate.update("insert into registry (in_number, in_date, customer, text_appeal, type_appeal, executor,employee,term_answer, deadline_term, note) values(?,?,?,?,?,?,?,?,?,?)",
                new Object[]{
                        appeal.getIn_number(),
                        appeal.getIn_date(),
                        appeal.getCustomer(),
    ...
    public RowMapper<Appeal> mapper = new RowMapper<Appeal>() {
        public Appeal mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            appeal.setNote(rs.getString("note"));
    ...
    public List<Appeal> reminderList() {
        ...
        String sql = "SELECT r.*, ta.name AS type_appeal_text, s.signature FROM registry r  " +
                " LEFT JOIN type_appeals ta ON r.type_appeal = ta.id  " +
                " LEFT JOIN signatures s on r.signature_id=s.id " +
                " WHERE DATEDIFF(r.deadline_term, now()) <= ta.reminder_term AND r.reminder_email IS NULL AND r.reminder_telephone IS NULL AND r.reminder_sc IS NULL and (r.out_number is  null or r.out_date is null)";
    ...
    public List<Appeal> expiredList() {
        ...
        String sql = "SELECT r.*, ta.name AS type_appeal_text, s.signature FROM registry r  " +
                ...
                " and r.deadline_term BETWEEN DATE_FORMAT(NOW() - INTERVAL 1 MONTH, '%Y-%m-01 00:00:00') AND NOW()";
    ...
    public void sendEmail(final String executor, final Integer dateDiff, final String in_number, final Date in_date, final String text_appeal, final String customer, final Date deadline_term, final String email) {
        ...
        mailSender.send(new MimeMessagePreparator() {
            ...
            public void prepare(MimeMessage mimeMessage) throws Exception {
                ...
                messageHelper.setFrom("control@oe.if.ua");
    ...
    RowMapper<TypeAppeal> ta_mapper = new RowMapper<TypeAppeal>() {
        public TypeAppeal mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            typeAppeal.setTerm_to_answer(rs.getInt("term_to_answer"));
            typeAppeal.setReminder_term(rs.getInt("reminder_term"));
            return typeAppeal;
        }
    };

    RowMapper<Signature> signaturemapper = new RowMapper<Signature>() {
